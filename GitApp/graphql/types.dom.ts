/**
 * This module contains types copied from lib.dom.d.ts that are in the type graph of the fetch utils.
 *
...
/** This Fetch utils interface allows you to perform various actions on HTTP request and response headers.
 * These actions include retrieving, setting, adding to, and removing.
 * A Headers object has an associated header list, which is initially empty and consists of zero or more name and
 * value pairs.  
...
interface URLSearchParams {
  ...
  has(name: string): boolean
  /**
   * Sets the value associated to a given search parameter to the given value. If there were several values,
   * delete the others.
   */
  set(name: string, value: string): void
  sort(): void
...
/** This Streams utils interface represents a readable stream of byte data. T
 * he Fetch utils offers a concrete instance of a ReadableStream through the body property of a Response object. */
...
interface Event {
   ...
   * but true can indicate that part of the operation during which event was dispatched,
   ...
   * except for any nodes in shadow trees of which the shadow root's mode is "closed"
   * that are not reachable from event's currentTarget.
   */
  composedPath(): EventTarget[]
  initEvent(type: string, bubbles?: boolean, cancelable?: boolean): void
  /**
   * If invoked when the cancelable attribute value is true,
   * and while executing a listener for the event with passive set to false, signals to the operation that caused event
...
export interface Request extends Body {
   ...
   * indicating how redirects for the request will be handled during fetching. A request will follow redirects by
   * default.
   */
...
/** This Streams utils interface provides a standard abstraction for writing streaming data to a destination, known as a
...
/** This Streams utils interface is the object returned by WritableStream.getWriter() and once created locks the < writer
